...
import { Ingredient, Nutrition, Recipe, RecipeGenerationRequest } from '../../../types/recipe';
...
export const mockedGenerateText = generateText as jest.MockedFunction<typeof generateText>;
 ...
 */
export const createMockNutrition = (overrides: Partial<Nutrition> = {}): Nutrition => ({
  calories: 250,
  protein: 12,
  carbs: 25,
  fat: 8,
 ...
 */
export const createMockRequest = (overrides: Partial<RecipeGenerationRequest> = {}): RecipeGenerationRequest => ({
  mealType: 'lunch',
  ...overrides
});
 ...
 */
export const mockSuccessfulGeneration = (recipe: Recipe = createMockRecipe()) => {
 ...
 */
export const createInvalidNutritionRecipe = (violation: 'negative_calories' | 'zero_protein' | 'negative_fat' = 'negative_calories'): Recipe => {
 ...
 */
export const createInvalidStructureRecipe = (violation: 'empty_ingredients' | 'empty_instructions' | 'missing_name' = 'empty_ingredients'): Partial<Recipe> => {
  const base = createMockRecipe();
...
export const mockErrorScenarios = {
  ...
  rateLimitExceeded: () => mockFailedGeneration(new Error('Rate limit exceeded')),
  invalidApiKey: () => mockFailedGeneration(new Error('Invalid API key')),
  modelOverloaded: () => mockFailedGeneration(new Error('Model is currently overloaded')),
...
export const assertNutritionalTargets = (
  ...
  if (targets.calories) {
    ...
    expect(recipe.nutrition.calories).toBeGreaterThanOrEqual(targets.calories - calorieRange);
    expect(recipe.nutrition.calories).toBeLessThanOrEqual(targets.calories + calorieRange);
  }
  ...
  if (targets.carbs) {
    ...
    expect(recipe.nutrition.carbs).toBeGreaterThanOrEqual(targets.carbs - carbRange);
    expect(recipe.nutrition.carbs).toBeLessThanOrEqual(targets.carbs + carbRange);
  }
